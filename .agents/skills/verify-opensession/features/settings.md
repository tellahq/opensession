...
- **Check phone navigation.** Open `/settings` at 390x844 and wait for `dialog` named `Settings`; the phone root does not expose a `Settings` heading. Choose a section, wait for button `Back to settings`, then use it to return to the list.
