...
- **Use search.** Return to `/settings`, fill the searchbox named `Search settings` (searchbox, not textbox) with `providers`, and choose the remaining `Providers` entry. The route becomes `/settings/providers` and the Providers heading appears.
...
- **Change instance configuration.** Use only the disposable demo instance. For a cheap end-to-end example, open `/settings/general`, fill textbox `Agent name` with a synthetic value, move focus off the field, and read `verify-opensession api "$RUN_ID" /api/settings/identity`. Its `personaName` carries the saved value and `configPath` points inside the run's disposable state directory. Never copy live credentials into this run.
- **Check phone navigation.** Open `/settings` at 390x844. The settings section list appears first, with no section heading. Choose a section, then use `Back to settings` to return to the list.
...
- Personal preferences may live in browser local storage while instance configuration lives in the disposable state directory. Verify the correct side effect for the control under test. Theme, for example, persists as the `opensession-theme` local storage key, so it survives navigation but is scoped to the run's browser profile.
