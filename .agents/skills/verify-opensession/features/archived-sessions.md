...
- **Clear and filter.** Refill the searchbox with an empty value, choose one of the current picker buttons such as `Owner, My archived`, and select a visible repository or person. Capture the filter state and narrowed result list.
