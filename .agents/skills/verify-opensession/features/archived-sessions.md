...
- **Clear and filter.** Refill the searchbox with an empty value, take a current snapshot, then choose the owner button whose name begins with `Owner,` and select `Everyone` or one visible person. If more than one repository is present, use the adjacent repository picker too. Capture the filter state and narrowed result list.
...
- Owner and repository filters are separate pickers whose accessible names include their current values. The repository picker is omitted when only one repository is available. Take a fresh snapshot after each change.
