...
- **Pause the created goal.** Open its row and choose `Pause` before the scheduler's first tick; confirm `/api/goals` reports `status: "paused"`. The scheduler runs even in the isolated demo, though executor work is disabled.
...
- Do not use `Run now`, `Resume`, or a wake as proof of engine behavior. The launcher disables executor work, not the scheduler. Pause any created active goal immediately.
