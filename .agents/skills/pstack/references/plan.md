...
Keep a short checklist with one item per step below.
...
Read the **Principles** section of the canonical `pstack` skill end to end, and the leaf `principle-*` skills it indexes. The principles govern every plan decision; cross-link them.
...
## 3. Explore in child sessions

Delegate codebase exploration when independent slices materially help (the **guard-the-context-window** principle skill).

- Discover policy-gated Open Session session tools and use ask-mode `spawn_task` children with self-contained `/pstack` briefs.
- Omit `model` unless the current workspace preset supplies an exact configured supporting model.
...
If a phase creates or edits a skill, the phase instructs the implementer to use the repository skill conventions for `SKILL.md` authoring.
