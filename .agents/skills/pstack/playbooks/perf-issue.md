...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to an isolated code child, omitting `model` unless the workspace preset provides one; review the diff. Capture a post-fix trace.
