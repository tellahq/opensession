...
**You own this task. Plan, review, verify.** Delegate investigation and the fix to child sessions, stay in the lead.
...
2. Binary-search the cause. Form the candidate hypotheses, then rule them out until one survives. Seed them with `how` over the affected subsystem and the **why** skill for regression history. Each pass, take the split that cuts the most remaining problem space, get runtime evidence, eliminate. When program state is unclear, add instrumentation or logging and read it as the code runs. Don't guess. Drive a long or stubborn hunt with Open Session sessions and explicit watcher commands. Confirm the surviving *mechanism* with runtime evidence before the step-3 architect/interrogate fan-out; a design grounded on a plausible-but-unconfirmed cause can be unanimously wrong while the real cause sits one subsystem over.
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation to an isolated code child with a specific scope, omitting `model` unless the workspace preset provides one; review the diff.
...
Investigation fans out `how` + `why` as parallel child sessions.
