...
- **changed** — one reviewable change contains proven doc, harness, or map corrections; publish it only when the repository workflow authorizes publication.
...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map (usually `.agents/skills/verify-*/`). Several candidates → ask which one; none → stop and point at `/skill:create-verification-skill` instead of inventing a target.
...
6. **Ship or stop.** For changed, re-read every changed file and follow the repository's commit, review, and publication workflow. For clean or blocked, create no change merely to produce an artifact; report the outcome and coverage honestly.
