---
name: automate-me
description: "Use for \"automate me\", \"create/update/refresh my -mode skill\", \"turn/capture my preferences or working style into a skill\", or wanting agents to follow how the user works. Drafts or revises a personal -mode skill via Open Session skill authoring + unslop, optionally pulling fresh evidence from recent transcripts."
...
- Delegation habits (child sessions, models, specialized workflows, parallelism)
...
Use the repository skill conventions to author the skill. Placement:

- Path: preserve an existing mode skill's category. For a new mode, use `.agents/skills/<handle>/<handle>-mode/SKILL.md` when the repo has an established personal category for that handle; otherwise default to `.agents/skills/<handle>-mode/SKILL.md` in the project. A personal skill still needs an organization-controlled tracked source; do not write host-global skill directories.
- Handle: the user's first name or chosen identifier.
- Frontmatter `description`: trigger on their name + `/<handle>-mode` + "work in their style", not on generic keywords like "write code" or "review PR".
- Frontmatter formatting: follow the repository's skill and YAML conventions. Keep `description` as one YAML scalar; quote it or use `description: >-` with indented continuation lines when punctuation or wrapping requires it.
...
Apply the **unslop** skill and the repository's skill-writing guidelines to every line. Both apply to any agent-read prose, not just skills.

Show the draft to the user and take feedback. Expect multiple iterations. Cut ruthlessly; a mode skill is not a manual.

### 6. Land it

Follow the repository's own branch, commit, review, and publication workflow. Do not create a PR, push, or alter shared worktrees unless the active user and repository instructions authorize it.
...
A `-mode` skill is subjective output. A benchmark loop is not useful unless trigger accuracy or behavior can be graded objectively. Vibe-check with the user: does it read like them? Did it miss anything? Then ship.

Run a description-optimization loop only if the skill's trigger accuracy turns out to be a problem in practice.

## When not to use

- User wants a task-specific skill (not working conventions): the repository skill conventions alone, no mining required.
...
- Repository skill conventions: authoring process and writing guidelines.
