...
Spawn a single ask-mode child session that explores and explains in one pass. Begin the brief with `/pstack`, omit `model` unless a configured preset supplies one, and do not grant write or edit tools.
...
Once all explorers return, spawn a single ask-mode child session to synthesize their findings into one coherent explanation. Begin the brief with `/pstack`, include only the reduced explorer findings, and do not grant write or edit tools.
...
After the explanation is complete, spawn up to four architectural critics in parallel. Use configured supporting model families when the current workspace preset provides them; otherwise omit explicit models.

Run each critic in ask mode with a self-contained `/pstack` brief. Use different configured supporting model families when the workspace preset provides them; otherwise omit `model`. Do not grant write or edit tools.
