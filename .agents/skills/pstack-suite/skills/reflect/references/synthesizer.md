...
- Existing-skill-first: propose `new skill via Open Session skill authoring:` only when no existing skill is a real home, the pattern recurs, and the topic deserves its own skill.
...
| <new pattern, no existing skill is a real home> | <draft a new skill via Open Session skill authoring> | <new skill via Open Session skill authoring: <kebab-name>> |
