...
Read the transcript digest below. Use policy-gated session/history tools only when the prompt supplies an exact session id.
...
- skill blocks or read calls against a project `.agents/skills/` or `.claude/skills/` `SKILL.md`
- child-session prompts that name a skill
- local or MCP tool calls that match a skill's documented actions
