...
At the end of the run, before handing back, check the log told the truth. Use the active conversation context. For an earlier session, use only policy-gated Open Session session or history tools with the exact session id. Never scan transcript files, databases, or global session storage. Walk the log against what actually happened:
...
Before handing back, spawn an ask-mode review child on a different configured model family when the workspace preset and session tools make one available. Otherwise use an inherited-model child and disclose that model diversity was unavailable. Give it the audit trail plus a reduced transcript digest, never a private transcript filesystem path. The child flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
