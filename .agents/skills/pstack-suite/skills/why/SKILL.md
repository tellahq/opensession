...
Before spawning investigators, use `mcp_search` to discover the policy-gated tools actually available in this Open Session run. Never inspect MCP configuration files, credentials, or host directories.
...
Discover the policy-gated Open Session session tools and launch all matching investigators concurrently with `spawn_task`. One investigator per category lets each specialize in one tool's query vocabulary and result shape. Do not ask one child to cover multiple MCPs.

Run every investigator in ask mode with a self-contained `/pstack` brief. Omit `model` unless the current workspace preset supplies a deliberate configured supporting model. Ask mode prevents repository writes while leaving the child session's policy-approved MCP reads available. Tool policy may still withhold a category; record that as a gap instead of bypassing it.
...
"It's pure feature code, error tracking won't have anything" is **not** sufficient, and neither is "I doubt long-form docs would have this." Run the search; let the null result speak. The cost of an investigator returning empty is one child session. The cost of missing a design doc that actually exists is a wrong answer.
...
Spawn one ask-mode synthesizer child with a self-contained `/pstack` brief. Omit `model` unless a configured workspace preset provides one. The synthesizer may use only its policy-gated MCP reads to spot-check citations.
...
- `references/investigator-prompt.md`. Base prompt template for investigator child sessions.
...
- `references/synthesizer-prompt.md`. Prompt template for the synthesizer child session, including the output format.
