...
Keep a checklist with one entry per phase before starting. Autonomous mode without checkpoints needs the list to show phase position and keep phases from silently disappearing.
...
Use the arena's Open Session model and child-session rules. Omit explicit models unless the current workspace preset provides configured supporting models.
