...
test("sidebar Changes shares Review's code display options", () => {
  ...
  expect(commentableDiffSource).toMatch(
    /:\s*utilityClassName\(\s*"w-auto rounded-none bg-\[var\(--review-file-header-bg\)\] hover:bg-\[var\(--review-file-header-hover\)\]"\s*,?\s*\)/,
  );
  expect(commentableDiffSource).toContain(
  ...
  expect(commentableDiffSource).toMatch(
    /!stickyFileHeaders\s*&&\s*utilityClassName\(\s*"mx-2 overflow-clip rounded-lg"\s*,?\s*\)/,
  );
  expect(commentableDiffSource).toMatch(
    /!stickyFileHeaders\s*&&\s*utilityClassName\(\s*"mt\-0\ rounded\-none"\s*,?\s*\)/,
  );
  expect(commentableDiffSource).toContain(
