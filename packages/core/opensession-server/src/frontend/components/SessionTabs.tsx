...
export function SessionTabs({
  ...
  const newTabButton = onNewSession && (
    <ContextMenu.Root>
      ...
      <ContextMenu.Popup
        className={mergeStylexOverrideClassName("", sx.minW250px)}
      >
  ...
  const historyMenu = hasHistory && (
    <Menu.Root>
      ...
      <Menu.Popup
        align="end"
        sideOffset={4}
        className={mergeStylexOverrideClassName("", sx.minW240px, sx.maxW320px)}
      >
  ...
  return (
    <div
      className={cn(
        TAB_STRIP,
        !inSplit && utilityClassName("desktop:-mt-[11px]"),
      )}
      role="tablist"
    >
      <div
                    ...
                    {v.icon ? (
                      <span
                        className={cn(
                          TAB_VICON,
                          v.closable !== false &&
                            utilityClassName("desktop:mr-3.5"),
                        )}
                        aria-hidden="true"
                      >
                        {v.icon}
