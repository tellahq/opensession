      ...
      <div {...stylex.props(sx.flex, sx.minW0, sx.itemsCenter, sx.gap7px)}>
        ...
        >
          <span {...stylex.props(sx.textGreen)}>
            +{compactNum(commit.additions)}
          </span>{" "}
          <span {...stylex.props(sx.textRed)}>
            -{compactNum(commit.deletions)}
          </span>
      ...
      >
        {commit.url && (
          <CardLink href={commit.url} title="Open on GitHub">
            <span {...stylex.props(sx.fontMono, sx.text095em)}>
              {commit.shortSha}
            </span>
          </CardLink>
        )}
...
function PrChipCardBody({ pr }: { pr: ChipPr }) {
  ...
  return (
    <>
      ...
      <div {...stylex.props(sx.flex, sx.minW0, sx.itemsCenter, sx.gap7px)}>
        ...
        >
          {pr.additions != null && pr.deletions != null ? (
            <>
              <span {...stylex.props(sx.textGreen)}>
                +{compactNum(pr.additions)}
              </span>{" "}
              <span {...stylex.props(sx.textRed)}>
                -{compactNum(pr.deletions)}
              </span>
            </>
          ) : (
            pr.branch
      ...
      <div
        className={utilityClassName(
          `mt-[3px] text-meta font-medium ${PR_STATE_TEXT[tone]}`,
        )}
      >
        {status.label}
      </div>
