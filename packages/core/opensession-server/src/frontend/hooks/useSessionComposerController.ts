...
import { setComposerMutableRef } from "../lib/session-viewer-send";
...
export function useComposerQueueState() {
  ...
  >(null);
  return { draggingQueueRef, pendingReorderRef };
}
...
interface FileDragRefs {
  dragDepthRef: MutableRefObject<number>;
  fileDragPresentRef: MutableRefObject<boolean>;
  cancelledFileDragRef: MutableRefObject<boolean>;
  fileDragWatchdogRef: MutableRefObject<ReturnType<typeof setTimeout> | null>;
}

interface FileDragState {
  setFileDragActive: Dispatch<SetStateAction<boolean>>;
}

export interface SessionFileDrag {
  refs: FileDragRefs;
  state: FileDragState;
}
...
export function usePendingPromptReconciliation({
  ...
  const reconcileOptimisticPrompts = useEffectEvent(() => {
  ...
  }, [entries, queued, steered, setPending]);
...
export function useSessionAttachmentDrop({
  ...
  function resetFileDrag() {
    setComposerMutableRef(refs.dragDepthRef, 0);
    state.setFileDragActive(false);
  }
  function finishFileDrag() {
    if (refs.fileDragWatchdogRef.current)
      clearTimeout(refs.fileDragWatchdogRef.current);
    setComposerMutableRef(refs.fileDragWatchdogRef, null);
    resetFileDrag();
    setComposerMutableRef(refs.fileDragPresentRef, false);
    setComposerMutableRef(refs.cancelledFileDragRef, false);
  }
  function armFileDragWatchdog() {
    if (refs.fileDragWatchdogRef.current)
      clearTimeout(refs.fileDragWatchdogRef.current);
    setComposerMutableRef(
      refs.fileDragWatchdogRef,
      setTimeout(finishFileDrag, 500),
    );
  }
  const dropAttachments = useEffectEvent((picked: FileList | File[]) =>
    addSessionAttachments(picked),
  );
  const listenForFileDrag = useEffectEvent(() => {
    ...
    function cancelFileDrag(event: KeyboardEvent) {
      if (event.key !== "Escape" || !refs.fileDragPresentRef.current) return;
      event.preventDefault();
      event.stopPropagation();
      setComposerMutableRef(refs.cancelledFileDragRef, true);
      resetFileDrag();
      armFileDragWatchdog();
    }
    ...
    function handleFileDragEnter(event: DragEvent) {
      ...
      event.preventDefault();
      armFileDragWatchdog();
      if (refs.cancelledFileDragRef.current) return;
      if (!refs.fileDragPresentRef.current) {
        setComposerMutableRef(refs.fileDragPresentRef, true);
        setComposerMutableRef(refs.dragDepthRef, 0);
      }
      setComposerMutableRef(refs.dragDepthRef, refs.dragDepthRef.current + 1);
      state.setFileDragActive(true);
    }
    function handleFileDragLeave(event: DragEvent) {
      ...
      if (refs.cancelledFileDragRef.current) {
      ...
      setComposerMutableRef(
        refs.dragDepthRef,
        Math.max(0, refs.dragDepthRef.current - 1),
      );
      if (refs.dragDepthRef.current === 0) state.setFileDragActive(false);
    }
    function handleFileDragOver(event: DragEvent) {
      ...
      if (!refs.fileDragPresentRef.current) {
        setComposerMutableRef(refs.fileDragPresentRef, true);
        setComposerMutableRef(refs.dragDepthRef, 1);
        state.setFileDragActive(true);
      }
    ...
    function handleFileDrop(event: DragEvent) {
      ...
      event.preventDefault();
      event.stopPropagation();
      const cancelled = refs.cancelledFileDragRef.current;
