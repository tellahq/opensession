...
export interface SessionModelWorkflowControllerOptions {
  session: UnifiedSession;
  dispatchSessionRuntime: Dispatch<SessionRuntimeAction>;
}

/**
 * What a send will run with — model, pinned account, effort, fast mode and the
 * optimistic goal — plus the dynamic workflow runs the Agents tab lists.
 *
 * Split out of SessionViewer verbatim, in the order the hooks ran there, so
 * hook order is unchanged and the memos over these values stay in the
 * component. The result is grouped rather than flat so each caller reads which
 * slice it depends on.
 */
export function useSessionModelWorkflowController({
  session,
  dispatchSessionRuntime,
}: SessionModelWorkflowControllerOptions) {
  ...
  // runs-vanished fallback must not flip tabs off an empty [] mid-fetch.
