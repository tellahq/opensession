...
export async function handleConnectionsRoutes(
  ...
  if (path === "/api/connections/mcp-oauth/callback" && req.method === "GET") {
    const code = url.searchParams.get("code") || "";
    const state = url.searchParams.get("state") || "";
    const { connectedPage, connectFailedPage } =
      await import("../connect-result-page");
    if (!code || !state)
      return connectFailedPage(
        undefined,
        "The redirect came back without a code, so nothing was connected.",
      );
    const { completeMcpOauthFlow, pendingFlowServer } =
  ...
  const mcpToolsMatch = path.match(/^\/api\/connections\/mcp\/([^/]+)\/tools$/);
  ...
  const mcpOauthMatch = path.match(/^\/api\/connections\/mcp\/([^/]+)\/oauth$/);
  ...
  const mcpTokenMatch = path.match(/^\/api\/connections\/mcp\/([^/]+)\/token$/);
  if (mcpTokenMatch && req.method === "POST") {
    ...
    if (!token)
      return Response.json({ error: "Paste an API token" }, { status: 400 });
    ...
    try {
      ...
      if (providerModels) {
        ...
        for (const m of providerModels) {
          ...
          let tail = m.trim();
          if (tail.startsWith("pi/")) tail = tail.slice("pi/".length);
  ...
  if (path === "/api/connections/github/device" && req.method === "POST") {
    const { startGithubDeviceFlow, githubConnectAvailable } =
      await import("../github-auth");
    const { webAuthRequired } = await import("../web-auth");
  ...
  if (path === "/api/connections/github/device/poll" && req.method === "POST") {
    const { pollGithubDeviceFlow, githubConnectAvailable } =
      await import("../github-auth");
    const { webAuthRequired } = await import("../web-auth");
    const simpleMode = !webAuthRequired();
    if (!githubConnectAvailable())
  ...
  if (ghAccountMatch && req.method === "DELETE") {
    ...
    const { removeGithubAccount, soleGithubLogin } =
      await import("../github-auth");
    const { webAuthRequired } = await import("../web-auth");
    const simpleMode = !webAuthRequired();
    if (!simpleMode) {
      ...
      if (
        !ctx.authUser?.login ||
        ctx.authUser.login.toLowerCase() !== login.toLowerCase()
      ) {
        return Response.json(
  ...
  if (path === "/api/connections/github/app" && req.method === "POST") {
    ...
    return withConfigMutationLock(async () => {
      ...
      const github = githubIntegrationSection(config, true)!;
      const keyMutation =
        privateKey || (github.oauthClientId !== clientId ? null : undefined);
      if (keyMutation === null) {
        return Response.json(
          {
            error:
  ...
  if (path === "/api/connections/plain-router" && req.method === "GET") {
  ...
  if (path === "/api/connections/plain-router" && req.method === "PUT") {
    const body = (await req.json().catch(() => null)) as {
      prompt?: string;
      basicModel?: string;
    } | null;
