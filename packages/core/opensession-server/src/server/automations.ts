 ...
 * is an agent run. Treating that as ownership would assign automations to
 * people who do not exist and hide them from every real one.
 *
...
export async function resumePendingAutomationRuns(
  ...
  // thread unless explicitly opted out (retriggers). Legacy intents still
  // collapse; only flagged ones are dropped for a thread that already has a
...
export async function runAutomation(
  ...
  options?: {
     ...
     * an earlier pending intent. Explicit retriggers set this to false: the
