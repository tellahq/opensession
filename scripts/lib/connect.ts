...
import {
  isCompiledBinary,
  runnerHostArgv,
} from "../../packages/core/opensession-server/src/runner-host/exe";

const IDENTITY_PATH = join(OPENSESSION_HOME, "runner.json");
const HEARTBEAT_MS = 60_000;
const RUNNER_HOST_ENTRY = resolve(
  import.meta.dir,
...
export function resolveWindowsSchtasks(
  probe: WindowsBinaryProbe = {},
): string | undefined {
  const exists = probe.exists ?? existsSync;
  const which = probe.which ?? ((binary: string) => Bun.which(binary));
...
export function runnerExecCommand(
  command: string,
...
async function detectResources(): Promise<Record<string, unknown>> {
  ...
  if (platform() === "win32") {
    const probe = runnerExecCommand(
      "(Get-PSDrive ((Get-Location).Drive.Name)).Free",
    );
    const free = Number(await commandOutput(probe));
    if (Number.isFinite(free) && free > 0)
      resources.freeDiskGb = Math.round((free / 1024 ** 3) * 10) / 10;
    // Silence here used to register a Runner with no freeDiskGb and no clue why.
    else
      warn("free disk space was not measured", `${probe[0]} did not report it`);
  } else {
    ...
    if (diskLine && Number.isFinite(Number(diskLine.at(-3)))) {
      resources.freeDiskGb =
        Math.round(((Number(diskLine.at(-3)) * 1024) / 1024 ** 3) * 10) / 10;
    }
  }
  const nvidia = await commandOutput([
    "nvidia-smi",
    "--query-gpu=name,memory.total,driver_version",
    "--format=csv,noheader,nounits",
  ]);
  if (nvidia) {
    const [model, vram, driver] = nvidia
      .split("\n")[0]
      .split(",")
      .map((part) => part.trim());
    const cudaVersion = (await commandOutput(["nvidia-smi"])).match(
      /CUDA Version:\s*([\d.]+)/,
    )?.[1];
    resources.gpu = {
      kind: "nvidia",
      model,
      vramGb: Number.isFinite(Number(vram))
        ? Math.round(Number(vram) / 102.4) / 10
        : undefined,
      driver,
      cuda: cudaVersion,
    };
  } else if (platform() === "darwin") {
    const displays = await commandOutput([
      "system_profiler",
      "SPDisplaysDataType",
      "-json",
    ]);
    try {
      const display = JSON.parse(displays).SPDisplaysDataType?.[0] as
        | Record<string, unknown>
        | undefined;
      if (display)
        resources.gpu = {
          kind: "apple",
          model: String(display.sppci_model ?? display._name ?? "Apple GPU"),
          metal: Boolean(display.spdisplays_metal),
        };
    } catch {}
  } else {
    const rocm = await commandOutput([
      "rocm-smi",
      "--showproductname",
      "--showmeminfo",
      "vram",
    ...
    if (rocm) {
      const model = rocm
        .split("\n")
        .find((line) => /card series|product name/i.test(line))
        ?.split(":")
        .at(-1)
        ?.trim();
      resources.gpu = {
        kind: "amd",
        ...(model ? { model } : {}),
        rocm: (await commandOutput(["rocm-smi", "--showversion"])).match(
          /[\d.]+/,
        )?.[0],
      };
    } else {
      ...
      const line = intel
        .split("\n")
        .find(
          (value) => /vga|3d controller/i.test(value) && /intel/i.test(value),
        );
      if (line)
        resources.gpu = {
          kind: "intel",
          model: line.split(":").slice(2).join(":").trim() || "Intel GPU",
        };
    }
  }
  ...
  if (ollama) {
    const models = ollama
      ...
      .filter((model) => model && model.length <= 160)
      .slice(0, 64);
    inference.push({ runtime: "ollama", models });
  }
  if (await commandOutput(["vllm", "--version"]))
    inference.push({ runtime: "vllm", models: [] });
...
export async function connect(opts: ConnectOptions): Promise<number> {
  ...
  if (!opts.code) {
    fail(
      "--code is required",
      "get one from the server with `opensession runners pair`",
    );
    return 1;
  }
  ...
  try {
    ...
    info(
      dim(
        "  the server must be reachable from this machine — usually the tailnet",
      ),
    );
    return 1;
  }
  ...
  if (!response.ok) {
    ...
    if (response.status === 403) {
      info(
        dim(
          "  either the pairing code is wrong/expired, or this machine is not",
        ),
      );
  ...
  const { runner, token } = (await response.json()) as {
    runner: { id: string };
    token: string;
  };
  ...
  await Bun.write(
    IDENTITY_PATH,
    JSON.stringify({ server, id: runner.id, token, name }, null, 2) + "\n",
  );
  ...
  heading("Next");
  info(
    `${bold("opensession runner run")}    hold the outbound control channel open`,
  );
  info(
    dim(
      "  install a service manager, then run `opensession runner service install`",
    ),
  );
  return 0;
}
 ...
 */
export function repairWindowsPath(
  ...
  const missing = windowsCorePathDirs(systemRoot).filter(
    (dir) => !present.has(dir.toLowerCase()),
  );
...
export async function installRunnerService(): Promise<boolean> {
  try {
    if (platform() === "darwin") {
      ...
      await run(
        ["launchctl", "bootout", `gui/${uid}/${RUNNER_SERVICE_LABEL}`],
        { quiet: true },
      );
      ...
      if (started.code !== 0)
        throw new Error(started.stderr || "launchctl bootstrap failed");
    ...
    if (platform() === "linux") {
      if (!Bun.which("systemctl")) {
        fail(
          "Runner service was not installed",
          "systemctl is not on this machine, so there is no user service manager to install into",
        );
        return false;
      }
      const path = join(
      ...
      if (reload.code !== 0 || enabled.code !== 0)
        throw new Error(
          enabled.stderr || reload.stderr || "systemctl user service failed",
        );
      ok(
        "Runner service installed",
        "systemd user service reconnects after restart",
      );
      return true;
    }
    if (platform() === "win32") {
      ...
      if (!schtasks) {
        ...
        info(
          dim(
            "  PATH is probably missing %SystemRoot%\\System32. Repair it, then run this again.",
          ),
        );
        return false;
      }
      ...
      const body = Buffer.concat([
        ...
        Buffer.from(runnerScheduledTaskXml(), "utf16le"),
      ]);
      await Bun.write(path, new Uint8Array(body));
      const created = await run(
        [schtasks, "/Create", "/TN", RUNNER_TASK_NAME, "/XML", path, "/F"],
        { quiet: true },
      );
      if (created.code !== 0)
        throw new Error(
          created.stderr || created.stdout || "schtasks create failed",
        );
    ...
    fail(
      "Runner service was not installed",
      `${platform()} has no supported per-user service manager`,
    );
  } catch (error) {
    warn(
      "Runner service was not installed",
...
export async function runnerRun(): Promise<number> {
  ...
  const wsUrl =
    identity.server.replace(/^http/, "ws") +
    `/runner-ws?id=${encodeURIComponent(identity.id)}`;
  let attempt = 0;
  let stopping = false;
  ...
  const connectOnce = () =>
    new Promise<void>((resolve) => {
      ...
      socket.addEventListener("open", async () => {
        ...
        const report = async () => {
          ...
          return {
            capabilities: {
              platform: platform(),
              toolchains: await detectCapabilities(),
              tags: [],
              hardware: resources,
            },
            resources,
        ...
        const heartbeat = setInterval(() => {
          void report()
            .then((next) =>
              socket.send(JSON.stringify({ t: "heartbeat", ...next })),
            )
            .catch(() => {});
        }, HEARTBEAT_MS);
      ...
      socket.addEventListener("message", async (event: any) => {
        ...
        if (msg?.t === "cancel") {
          const proc =
            running.get(String(msg.id)) || persistent.get(String(msg.id));
          if (proc) proc.kill();
          return;
        }
        if (
          msg?.t === "workspace_prepare" &&
          msg.version === 1 &&
          msg.operationToken
        ) {
        ...
        if (
          ["portal_ws_open", "portal_ws_send", "portal_ws_close"].includes(
            msg?.t,
          ) &&
          msg.version === 1
        ) {
      ...
      socket.addEventListener("close", (event: any) => {
        ...
        if (event?.code === 1008 || event?.code === 4401) {
          fail(
            "the server refused this Runner",
            "its credential may have been revoked",
          );
          stopping = true;
        }
        for (const proc of running.values()) proc.kill();
        for (const portal of portalSockets.values()) {
          try {
            portal.close();
          } catch {}
        }
...
async function prepareWorkspace(socket: WebSocket, msg: any): Promise<void> {
  ...
  const repositoryUrl =
    typeof msg.repositoryUrl === "string" ? msg.repositoryUrl : "";
  ...
  try {
    ...
    if (!workspacePath || !absolute || workspacePath.includes("\0"))
      throw new Error("Invalid managed workspace path");
    if (
      !/^https:\/\/github\.com\/[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+\.git$/.test(
        repositoryUrl,
      )
    )
      throw new Error("Runner only accepts an approved GitHub repository URL");
    ...
    try {
      if (existsSync(workspacePath)) {
        const origin = await runnerCommand(
          ["git", "-C", workspacePath, "remote", "get-url", "origin"],
          env,
        );
        if (origin.code !== 0 || origin.stdout.trim() !== repositoryUrl)
          throw new Error(
            "Managed workspace does not match this session repository",
          );
        const fetch = await runnerCommand(
          ["git", "-C", workspacePath, "fetch", "--prune", "origin"],
          env,
        );
        if (fetch.code !== 0)
          throw new Error(
            fetch.stderr.trim() || "Could not refresh managed workspace",
          );
        const remoteBranch = await runnerCommand(
          [
            "git",
            "-C",
            workspacePath,
        ...
        if (checkout.code !== 0)
          throw new Error(
            checkout.stderr.trim() || "Could not check out managed branch",
          );
      } else {
        mkdirSync(dirname(workspacePath), { recursive: true });
        const clone = await runnerCommand(
          ["git", "clone", repositoryUrl, workspacePath],
          env,
        );
        if (clone.code !== 0)
          throw new Error(
            clone.stderr.trim() || "Could not clone managed workspace",
          );
        const remoteBranch = await runnerCommand(
          [
            "git",
...
async function cleanupWorkspace(socket: WebSocket, msg: any): Promise<void> {
  ...
  try {
    ...
    if (
      !absolute ||
      !sessionId ||
      !/^[A-Za-z0-9_-]{3,128}$/.test(sessionId) ||
      normalized.split(/[\\/]/).at(-1) !== sessionId ||
      normalized.split(/[\\/]/).at(-2) !== "sessions"
    )
      throw new Error("Invalid managed workspace cleanup path");
    if (existsSync(workspacePath))
      rmSync(workspacePath, { recursive: true, force: true });
    socket.send(
      JSON.stringify({ t: "workspace_cleaned", id, operationToken, ok: true }),
    );
  } catch (error) {
    socket.send(
      JSON.stringify({
...
async function scopedGitEnvironment(
  cloneToken: string,
  ...
  return {
    env: {
      ...env,
      GIT_ASKPASS: askpass,
      OPENSESSION_RUNNER_GIT_TOKEN: cloneToken,
    },
...
export function parseRunnerPortalRegistry(
  contents: string,
): RunnerPortalRecord[] {
  ...
  for (const line of contents.split("\n")) {
    ...
    try {
      const portal = JSON.parse(
        line.slice(RUNNER_PORTAL_PREFIX.length),
      ) as RunnerPortalRecord;
      if (
        !portal ||
        !PORTAL_NAME.test(portal.name) ||
        !Number.isInteger(portal.port) ||
        portal.port < PORTAL_MIN ||
        portal.port > PORTAL_MAX ||
        typeof portal.command !== "string"
      )
        continue;
...
function readRunnerPortalRegistry(workspacePath: string): RunnerPortalRecord[] {
  try {
    return parseRunnerPortalRegistry(
      readFileSync(runnerPortalRegistryPath(workspacePath), "utf8"),
    );
  } catch {
    return [];
  }
...
export function serializeRunnerPortalRegistry(
  previousText: string,
  portals: RunnerPortalRecord[],
): string {
  const generatedKeys = new Set(
    portals.map((portal) => runnerPortalKey(portal.name)),
  );
...
function writeRunnerPortalRegistry(
  ...
  writeFileSync(path, serializeRunnerPortalRegistry(previous, portals), {
    mode: 0o600,
  });
}
...
function runnerPortalKey(name: string): string {
  return `PORTAL_${name.toUpperCase().replace(/-/g, "_")}_PORT`;
}

function runnerPortalUpsert(
  records: RunnerPortalRecord[],
  next: RunnerPortalRecord,
): RunnerPortalRecord[] {
...
async function runnerPortalStatus(
  workspacePath: string,
...
async function startRunnerPortal(
  workspacePath: string,
  msg: any,
): Promise<RunnerPortalRecord> {
  const name =
    typeof msg.name === "string" ? msg.name.trim().toLowerCase() : "";
  const command = typeof msg.command === "string" ? msg.command.trim() : "";
  if (!PORTAL_NAME.test(name) || !command || command.length > 8_000)
  ...
  if (current && current.state !== "failed" && current.state !== "stopped")
    throw new Error(`Portal '${name}' already exists.`);
  let port = typeof msg.port === "number" ? msg.port : 0;
  if (port) {
    if (!Number.isInteger(port) || port < PORTAL_MIN || port > PORTAL_MAX)
      throw new Error("Invalid Portal port.");
    if (
      records.some((record) => record.name !== name && record.port === port) ||
      (await runnerPortListening(port))
    )
      throw new Error(`Port ${port} is already in use.`);
  } else {
    for (port = 4_000; port < 9_000; port++)
      if (
        !records.some((record) => record.port === port) &&
        !(await runnerPortListening(port))
      )
        break;
  ...
  const base: RunnerPortalRecord = {
    name,
    key: runnerPortalKey(name),
    command,
    port,
    ...(typeof msg.description === "string" && msg.description.trim()
      ? { description: msg.description.trim().slice(0, 240) }
      : {}),
    ...(typeof msg.portalUrl === "string" &&
    msg.portalUrl.startsWith("https://")
      ? { portalUrl: msg.portalUrl }
      : {}),
    state: "starting",
  ...
  const env = {
    ...runnerEnvironment(),
    PORT: String(port),
    PORTAL_URL: typeof msg.portalUrl === "string" ? msg.portalUrl : "",
    OPENSESSION_PORTAL: name,
  };
  const child =
    platform() === "win32"
      ? Bun.spawn(runnerExecCommand(command), {
          cwd: workspacePath,
          env,
          stdin: "ignore",
          stdout: "ignore",
      ...
      : Bun.spawn(["setsid", "bash", "-lc", `exec ${command}`], {
          cwd: workspacePath,
          env,
          stdin: "ignore",
          stdout: "ignore",
  ...
  writeRunnerPortalRegistry(
    workspacePath,
    runnerPortalUpsert(records, running),
  );
  ...
  while (Date.now() < deadline) {
    if (await runnerPortListening(port)) {
      ...
      writeRunnerPortalRegistry(
        workspacePath,
        runnerPortalUpsert(records, awake),
      );
      return awake;
    }
    await Bun.sleep(200);
...
async function allocateRunnerPortalPort(
  ...
  if (port) {
    if (
      ...
      throw new Error("Requested Portal port is unavailable.");
    return { port };
  }
  for (port = 4_000; port < 9_000; port++)
    if (
...
async function stopRunnerPortal(
  workspacePath: string,
  nameValue: unknown,
): Promise<RunnerPortalRecord> {
  const name =
    typeof nameValue === "string" ? nameValue.trim().toLowerCase() : "";
  const records = readRunnerPortalRegistry(workspacePath);
  const current = records.find((record) => record.name === name);
  ...
  if (current.pid && current.pid > 1) {
    try {
      process.kill(
        platform() === "win32" ? current.pid : -current.pid,
        "SIGTERM",
      );
    } catch {
      try {
  ...
  writeRunnerPortalRegistry(
    workspacePath,
    runnerPortalUpsert(records, stopped),
  );
  return stopped;
}

function setRunnerPortalPath(
  workspacePath: string,
  nameValue: unknown,
  pathValue: unknown,
): RunnerPortalRecord {
  ...
  writeRunnerPortalRegistry(
    workspacePath,
    runnerPortalUpsert(records, updated),
  );
  return updated;
}
...
async function runnerPortalHttp(
  workspacePath: string,
  msg: any,
  ...
  if (
    ...
    throw new Error("Invalid Portal HTTP request.");
  const portal = (await runnerPortalStatus(workspacePath)).find(
    (record) => record.port === port && record.state === "awake",
  );
  ...
  if (
    ...
    for (const [name, value] of Object.entries(
      ...
      if (
        !RELAY_HEADERS.has(name.toLowerCase()) &&
        typeof value === "string" &&
        value.length <= 8_192
      )
  ...
  if (typeof msg.body === "string" && msg.body) {
    ...
    if (body.byteLength > 5 * 1024 * 1024)
      throw new Error("Portal request body is too large.");
  }
  ...
  if (bytes.byteLength > 10 * 1024 * 1024)
    throw new Error("Portal response is too large.");
  const responseHeaders: Record<string, string> = {};
  for (const [name, value] of response.headers)
    if (!RELAY_HEADERS.has(name.toLowerCase())) responseHeaders[name] = value;
  return {
...
function handleRunnerPortalWebSocket(
  socket: WebSocket,
  portalSockets: Map<string, WebSocket>,
  msg: any,
): void {
  const connectionId =
  ...
  if (msg.t === "portal_ws_close") {
    try {
      portalSockets.get(connectionId)?.close();
    } catch {}
  ...
  if (msg.t === "portal_ws_send") {
    ...
    try {
      if (msg.binary === true && typeof msg.data === "string")
        portal.send(Buffer.from(msg.data, "base64"));
  ...
  void (async () => {
    try {
      ...
      const remote = new WebSocket(`ws://127.0.0.1:${port}${path}`, {
        headers: forwarded,
      } as any);
      portalSockets.set(connectionId, remote);
      remote.addEventListener("open", () => {
        try {
          socket.send(JSON.stringify({ t: "portal_ws_opened", connectionId }));
        } catch {}
      });
      ...
      const close = () => {
        portalSockets.delete(connectionId);
        try {
          socket.send(JSON.stringify({ t: "portal_ws_closed", connectionId }));
        } catch {}
      };
...
async function handleRunnerPortal(socket: WebSocket, msg: any): Promise<void> {
  ...
  try {
    ...
    if (
      ...
      throw new Error("Invalid Runner Portal workspace.");
    let result: unknown;
    if (msg.t === "portal_allocate")
      result = await allocateRunnerPortalPort(workspacePath, msg.port);
    else if (msg.t === "portal_start")
      result = await startRunnerPortal(workspacePath, msg);
    else if (msg.t === "portal_list")
      result = await runnerPortalStatus(workspacePath);
    else if (msg.t === "portal_stop")
      result = await stopRunnerPortal(workspacePath, msg.name);
    else if (msg.t === "portal_path")
      result = setRunnerPortalPath(workspacePath, msg.name, msg.path);
    else if (msg.t === "portal_restart") {
      const existing = await stopRunnerPortal(workspacePath, msg.name);
      result = await startRunnerPortal(workspacePath, {
        ...existing,
        ...(typeof msg.portalUrl === "string"
          ? { portalUrl: msg.portalUrl }
          : {}),
      });
    } else if (msg.t === "portal_http")
      result = await runnerPortalHttp(workspacePath, msg);
...
async function handleRunnerTerminal(
  socket: WebSocket,
  terminals: Map<string, RunnerTerminalProcess>,
  msg: any,
): Promise<void> {
  const id =
    typeof msg.id === "string" && /^rt\d+-[A-Za-z0-9_-]{16}$/.test(msg.id)
      ? msg.id
      : "";
  if (!id) return;
  if (msg.t === "terminal_input") {
    if (typeof msg.data === "string" && msg.data.length <= 128_000) {
      try {
        (terminals.get(id) as any)?.terminal?.write(
  ...
  if (msg.t === "terminal_resize") {
    try {
      (terminals.get(id) as any)?.terminal?.resize(
        terminalSize(msg.cols, 100, 20, 500),
        terminalSize(msg.rows, 30, 5, 200),
      );
    } catch {}
    return;
  }
  ...
  try {
    ...
    const proc = Bun.spawn(argv, {
      ...
      terminal: {
        ...
        rows: terminalSize(msg.rows, 30, 5, 200),
    ...
    void proc.exited.then((code) => {
      terminals.delete(id);
      try {
        socket.send(JSON.stringify({ t: "terminal_exit", id, code }));
      } catch {}
    });
    socket.send(
      JSON.stringify({
...
async function startRunHost(
  socket: WebSocket,
  persistent: Map<string, ReturnType<typeof Bun.spawn>>,
  msg: any,
): Promise<void> {
  const id = String(msg.id);
  const token = String(msg.operationToken);
  try {
    ...
    if (
      !spec ||
      typeof spec !== "object" ||
      typeof spec.hostId !== "string" ||
      typeof spec.cwd !== "string" ||
      !spec.wsToken
    )
      throw new Error("Invalid run-host request");
    ...
    // host.ts on disk.
    if (!isCompiledBinary() && !existsSync(RUNNER_HOST_ENTRY))
      throw new Error(
        "This Runner installation does not include the run-host entrypoint",
      );
    ...
    const base = String(msg.server || "")
      .replace(/\/$/, "")
      .replace(/^http/, "ws");
    if (!/^wss?:\/\//.test(base))
      throw new Error("Invalid Open Session endpoint");
    const env = {
      PATH: process.env.PATH || "/usr/local/bin:/usr/bin:/bin",
      HOME: process.env.HOME || "/tmp",
    ...
    void proc.exited.finally(async () => {
      ...
      try {
        socket.send(JSON.stringify({ t: "host_exited", hostId: spec.hostId }));
      } catch {}
    });
    socket.send(
      JSON.stringify({
...
async function reportRunHostStatus(
  socket: WebSocket,
  persistent: Map<string, ReturnType<typeof Bun.spawn>>,
  msg: any,
): Promise<void> {
  ...
  let alive = false;
  try {
    if (!hostId || !workspacePath || workspacePath.includes("\0"))
      throw new Error("Invalid host status request");
    ...
    if (!alive) {
      ...
      const spec = JSON.parse(
        await Bun.file(join(stateDir, "spec.json")).text(),
      ) as { hostId?: string };
      const pid = Number((await Bun.file(join(stateDir, "pid")).text()).trim());
      if (spec.hostId !== hostId || !Number.isSafeInteger(pid) || pid <= 0)
...
export async function runnerStatus(): Promise<number> {
  ...
  info(
    dim(
      `  capabilities: ${(await detectCapabilities()).join(", ") || "none detected"}`,
    ),
  );
  return 0;
}
...
export async function runnersPair(): Promise<number> {
  ...
  info(
    dim(
      "  (Windows: irm https://raw.githubusercontent.com/tellahq/opensession/main/install.ps1 | iex)",
    ),
  );
  info(
    `  opensession connect --server ${(await localApi()).replace("/api/runners", "")} --code ${result.code}`,
  );
  return 0;
}
...
export async function runnersList(): Promise<number> {
  ...
  for (const runner of runners) {
    const seen = runner.lastSeenAt
      ...
      : "never connected";
    info(
      `${runner.name}  ${dim(`${runner.platform}/${runner.arch}`)}  ${runner.state}`,
    );
    info(dim(`  ${runner.id}  ${runner.address}  ${seen}`));
    if (runner.capabilities?.toolchains?.length)
      info(dim(`  can: ${runner.capabilities.toolchains.join(", ")}`));
  }
  return 0;
}
