...
function mockClient(
  ...
  return new FathomClient("test-token-never-real", "ABC123", {
...
describe("Fathom MCP contracts", () => {
  ...
  test("validates Gregorian dates, ordering, and range cap", () => {
    expect(validateRange("2024-02-29", "2024-03-01")).toEqual({
      from: "2024-02-29",
      to: "2024-03-01",
    });
    expect(() => validateRange("2023-02-29", "2023-03-01")).toThrow(
      "not a valid date",
    );
    expect(() => validateRange("2024-03-02", "2024-03-01")).toThrow(
      "must not be after",
    );
    expect(() => validateRange("2024-01-01", "2025-01-01")).toThrow(
      "cannot exceed 366",
    );
  });
  ...
  test("computes deterministic prior periods and clamps leap day", () => {
    ...
    expect(
      previousRange({ from: "2024-02-29", to: "2024-03-01" }, "previous_year"),
    ).toEqual({ from: "2023-02-28", to: "2023-03-01" });
  });
  ...
  test("parses finite numeric strings and preserves absent values", () => {
    ...
    expect(() => parseNumeric("9007199254740992", "visits")).toThrow(
      "out-of-range",
    );
  });
  ...
  test("defines zero-baseline comparison semantics", () => {
    ...
    expect(delta(0, 0)).toEqual({
      absolute: 0,
      percent: 0,
      status: "unchanged",
    });
    expect(delta(15, 10)).toEqual({
      absolute: 5,
      percent: 50,
      status: "increase",
    });
...
describe("FathomClient", () => {
  test("pins site lookup and uses only GET with bearer auth", async () => {
    ...
    expect(await client.call("list_sites", {})).toEqual({
      sites: [site],
      pinned_site_id: "ABC123",
    });
    expect(seen?.url.toString()).toBe(
      "https://api.usefathom.com/v1/sites/ABC123",
    );
    expect(seen?.init.method).toBe("GET");
    expect((seen?.init.headers as Record<string, string>).Authorization).toBe(
      "Bearer test-token-never-real",
    );
    await expect(
      client.call("list_sites", { site_id: "OTHER" }),
  ...
  test("builds exact summary totals/trend queries and caches successful single-flight calls", async () => {
    ...
    const [first, second] = await Promise.all([
      client.call("traffic_summary", args),
      client.call("traffic_summary", args),
    ]);
    ...
    expect(
      urls.some(
        (value) =>
          value.includes(
            "aggregates=visits%2Cuniques%2Cpageviews%2Cavg_duration%2Cbounce_rate",
          ) &&
          value.includes("date_from=2026-08-01+00%3A00%3A00") &&
          value.includes("date_to=2026-08-07+23%3A59%3A59"),
      ),
    ).toBe(true);
    expect(urls.some((value) => value.includes("date_grouping=day"))).toBe(
      true,
    );
  });
  ...
  test("builds safe breakdown grouping/filter and joins comparison ranks", async () => {
    ...
    const client = mockClient((url) => {
      urls.push(url);
      if (url.pathname.endsWith("/sites/ABC123")) return json(site);
      if (url.searchParams.get("date_from")?.startsWith("2026-08-01"))
        return json([
          { pathname: "/a", visits: "9" },
          { pathname: "/new", visits: "3" },
        ]);
      return json([
        { pathname: "/a", visits: "6" },
        { pathname: "/gone", visits: "2" },
      ]);
    });
    ...
    expect(result.dropped).toEqual([
      { key: "/gone", prior_rank: 2, prior_value: 2 },
    ]);
    ...
    expect(JSON.parse(report.searchParams.get("filters")!)).toEqual([
      { property: "pathname", operator: "is", value: "/pricing" },
    ]);
  });
  ...
  test("retries only 429/503 and honors bounded Retry-After", async () => {
      ...
      () => {
        ...
        if (calls < 3)
          return new Response("busy", {
            ...
            headers: { "retry-after": calls === 1 ? "2" : "999" },
          });
        return json({ total: 4 });
      },
      {
    ...
    const unauthorized = mockClient(() => {
      unauthorizedCalls++;
      return new Response("no", { status: 401 });
    });
    await expect(unauthorized.call("current_visitors", {})).rejects.toThrow(
      "returned 401",
    );
    ...
    const client = mockClient((_url, init) => {
      ...
      if (calls === 1)
        throw new DOMException(
          "timed out with secret-token-abcdefghijklmnopqrstuvwxyz",
          "AbortError",
        );
      return json({ total: 1 });
    });
    await expect(client.call("current_visitors", {})).rejects.toThrow(
      "Fathom request failed",
    );
  ...
  test("caps detailed live output and reports truncation", async () => {
    const content = Array.from({ length: 51 }, (_, index) => ({
      hostname: "example.com",
      pathname: `/p${index}`,
      total: index,
    }));
    const client = mockClient(() =>
      json({ total: 51, content, referrers: [] }),
    );
    const result = (await client.call("current_visitors", {
      detailed: true,
    })) as any;
  ...
  test("rejects malformed payloads, invalid filters, and oversized trend combinations", async () => {
    const client = mockClient((url) =>
      url.pathname.endsWith("/sites/ABC123")
        ? json(site)
        : json([{ visits: "oops" }]),
    );
    await expect(
      client.call("traffic_breakdown", {
        date_from: "2026-08-01",
        date_to: "2026-08-02",
        dimension: "page",
        path: "not-a-path",
      }),
    ).rejects.toThrow("path must begin");
    await expect(
      client.call("traffic_summary", {
        date_from: "2026-01-01",
        date_to: "2026-01-08",
        granularity: "hour",
      }),
    ).rejects.toThrow("limited to 7 days");
  });
  ...
  test("redacts and truncates secrets in errors", () => {
    ...
    const output = sanitizeError(
      new Error(
        `Authorization: Bearer ${secret} https://x.test?a=1&token=${secret} ${"x".repeat(400)}`,
      ),
    );
