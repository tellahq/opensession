 ...
 *   engine-seed/         opencode's own first-run install, prebuilt: the
 *                        `@opencode-ai/plugin@<pinned engine version>` npm
 *                        tree (package.json, package-lock.json, node_modules)
 *                        it would otherwise arborist-install into
 *                        ~/.config/opencode on the first session, ~40s on a
 *                        fresh box. install.sh copies it in; opencode skips
 *                        the install when node_modules exists and the lock
 *                        names the dependency (packages/core/src/npm.ts).
 *                        Needs `npm` on the build host (arborist reads an npm
 *                        lockfile, which bun does not write); skipped with a
 *                        warning otherwise.
 *   release.json         version, commit, target, bun version, engine
 *                        version, built-at; its presence puts the server in
...
import { FRONTEND_DIST, compileAssets } from "../src/server/frontend-build";
...
// The engine version install.sh pins (package.json `opensession.opencodeVersion`);
// the seed below must match it exactly, since opencode asks for its own version.
...
step(`engine seed (@opencode-ai/plugin@${engineVersion ?? "?"})`);
if (!engineVersion) {
  console.log("no opensession.opencodeVersion in package.json; skipping");
} else if (!Bun.which("npm")) {
  console.log("npm not on PATH; skipping (first session on the box will install it)");
} else {
  ...
  // Same flags opencode's arborist call uses (ignoreScripts, exact save), plus
  // the target platform so the optional native packages match the box.
  await sh(
    ["npm", "install", "--ignore-scripts", "--no-audit", "--no-fund", "--save-exact", "--loglevel=error", `--os=${os}`, `--cpu=${arch}`],
    { cwd: seed },
  );
  console.log(`${readdirSync(join(seed, "node_modules")).length} packages, ${(dirBytes(seed) / 1e6).toFixed(0)} MB`);
}
...
writeFileSync(
  ...
  JSON.stringify(
    { name, version, commit, os, arch, bun: BUN_VERSION, opencode: engineVersion ?? null, builtAt: new Date().toISOString() },
    null,
    2,
  ) + "\n",
);
