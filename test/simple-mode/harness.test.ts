...
async function host(cmd: string[], opts: { cwd?: string } = {}): Promise<Run> {
  const p = Bun.spawn(cmd, { cwd: opts.cwd, stdin: "ignore", stdout: "pipe", stderr: "pipe" });
  ...
  if (code !== 0 && !stdout && !stderr) {
    console.log(`[harness] ${cmd[0]} exited ${code} with no output (which=${Bun.which(cmd[0])})`);
  }
...
async function guest(line: string, opts: { user?: string } = {}): Promise<Run> {
  ...
  if (TARGET === "host") {
    const cmd = user ? ["sudo", "-u", user, "-i", "bash", "-lc", line] : ["bash", "-lc", line];
    return host(cmd);
  }
  const cmd = user
...
function expectOk(r: Run, what: string) {
  if (r.code !== 0) {
    throw new Error(`${what} failed (${r.code})\n--- stdout\n${r.stdout}\n--- stderr\n${r.stderr}`);
  }
}
...
describe("simple mode install", () => {
  beforeAll(async () => {
    ...
    for (const f of readdirSync(RELEASE_OUT).filter((f) => f.endsWith(".tar.gz"))) {
      rmSync(join(RELEASE_OUT, f), { force: true });
    }
  }, 2 * MINUTES);

  test("build: the release artefact (or a source bundle) for the guest", async () => {
    await Bun.write(join(WORK, "install.sh"), Bun.file(join(REPO_ROOT, "install.sh")));
    for (const f of ["goss.yaml", "goss.dod.yaml", "goss.uninstalled.yaml"]) {
      await Bun.write(join(WORK, f), Bun.file(join(HERE, f)));
    }
    if (SOURCE) {
      const b = await host(["git", "rev-parse", "--abbrev-ref", "HEAD"], { cwd: REPO_ROOT });
    ...
    const arch = TARGET === "host" ? process.arch : (process.arch === "arm64" ? "arm64" : "x64");
    const os = TARGET === "host" && process.platform === "darwin" ? "darwin" : "linux";
    ...
    console.log(`artefact ${tarballName} (${(statSync(join(WORK, tarballName)).size / 1e6).toFixed(0)} MB)`);
  }, 20 * MINUTES);

  test("vm: fresh Ubuntu via Lima", async () => {
    if (TARGET === "host") return;
    if ((await limaExists()) && !REUSE) {
      expectOk(await host(["limactl", "delete", "--force", VM]), "limactl delete");
    }
    if (!(await limaExists())) {
      const r = await host([
      ...
      expectOk(await host(["limactl", "start", VM]), "limactl start (existing)");
    }
    const probe = await guest(`test -f ${guestWork}/install.sh`);
    expectOk(probe, "work dir mounted in guest");
  }, 15 * MINUTES);

  test("goss: present in the guest", async () => {
    ...
    const cached = join(RELEASE_OUT, "..", `goss-${ver}-${gossOs}-${gossArch}`);
    if (!existsSync(cached)) {
      ...
      const dl = await host(["curl", "-fsSL", "--retry", "5", "--retry-delay", "10", "--retry-all-errors", "-o", tgz, url]);
      expectOk(dl, `download ${url}`);
      const un = await host(["tar", "-xzf", tgz, "-C", dirname(cached), "goss"]);
  ...
  test("install: install.sh from the bundle, non-interactive", async () => {
    ...
    const tokenEnv = CLAUDE_TOKEN ? `OPENSESSION_CLAUDE_TOKEN=${JSON.stringify(CLAUDE_TOKEN)} ` : "";
    const r = await guest(
  ...
  test("goss: today's bar", async () => {
    const tap = await goss("goss.yaml");
    console.log(tap);
  }, 2 * MINUTES);

  test("session: the default repo yields a worktree", async () => {
    ...
    // one, and that is fine here.
    //
    // Also the engine cold-start check: an artefact install seeds opencode's
    // plugin runtime (~/.config/opencode) so the first instance load does not
    // npm-install it. If opencode re-installs anyway, arborist rewrites the
    // lockfile; compare its mtime across the first session.
    const lock = "~/.config/opencode/package-lock.json";
    const seeded = !SOURCE && (await guest(`test -f ${lock}`)).code === 0;
    if (!SOURCE && !seeded) throw new Error("artefact install did not seed ~/.config/opencode (engine-seed missing?)");
    const lockBefore = seeded ? (await guest(`stat -c %Y ${lock}`)).stdout.trim() : "";
    ...
    let last = "";
    while (Date.now() < deadline) {
      const r = await guest(`curl -fsS http://127.0.0.1:${PORT}/api/sessions/${id}`);
      const s = r.code === 0 ? (JSON.parse(r.stdout) as { worktreeDir?: string; lastRunError?: { message?: string } }) : {};
      const err = s.lastRunError?.message ?? "";
      if (/Session setup failed/i.test(err)) throw new Error(`session setup failed: ${err}`);
      if (s.worktreeDir) {
        const wt = await guest(`test -e ${JSON.stringify(s.worktreeDir)}/.git`);
        if (wt.code === 0) {
          if (seeded) {
            // The instance loads (and would install) as soon as the run starts;
            // give it a moment past worktree creation, then compare.
            await Bun.sleep(15_000);
            const lockAfter = (await guest(`stat -c %Y ${lock}`)).stdout.trim();
            if (lockAfter !== lockBefore) {
              throw new Error(`opencode re-installed its plugin runtime despite the seed (lock mtime ${lockBefore} -> ${lockAfter})`);
            }
            console.log("engine plugin runtime seed held (no install on first session)");
          }
          return;
        }
      }
  ...
  test("turn: one real agent turn (needs OPENSESSION_TEST_CLAUDE_TOKEN)", async () => {
    if (!CLAUDE_TOKEN) {
    ...
    const status = await guest(`curl -fsS http://127.0.0.1:${PORT}/api/settings/opencode-engine`);
    expectOk(status, "engine status");
    const st = JSON.parse(status.stdout) as { ready?: boolean; claudeAccounts?: number; blocker?: string };
    if (!st.ready) throw new Error(`engine not ready after token import: ${st.blocker} (accounts=${st.claudeAccounts})`);
    expectOk(await guest("test ! -e ~/.opensession-claude-token"), "token file removed after import");
    ...
    while (Date.now() < deadline) {
      const r = await guest(`curl -fsS http://127.0.0.1:${PORT}/api/sessions/${id}`);
      const s = r.code === 0 ? (JSON.parse(r.stdout) as { runState?: string; lastRunError?: { message?: string } }) : {};
      if (s.lastRunError?.message) throw new Error(`turn failed: ${s.lastRunError.message}`);
      if (s.runState === "failed") throw new Error(`turn failed: ${r.stdout}`);
      const t = await guest(`curl -fsS http://127.0.0.1:${PORT}/api/sessions/${id}/transcript`);
      const entries = t.code === 0 ? (JSON.parse(t.stdout) as { type?: string; content?: string }[]) : [];
      const reply = entries.find((e) => e.type === "assistant" && (e.content ?? "").trim());
      if (reply) {
        console.log(`first turn completed in ${((Date.now() - t0) / 1000).toFixed(0)}s: ${JSON.stringify((reply.content ?? "").slice(0, 80))}`);
        return;
      }
      last = r.stdout;
      await Bun.sleep(3000);
  ...
  test("goss: definition of done (STRICT)", async () => {
    if (!STRICT) {
  ...
  test("reboot: comes back on its own (STRICT, lima)", async () => {
  ...
  test("uninstall: committed scratch work blocks removal (STRICT)", async () => {
    if (!STRICT) {
      console.log("skipped: SIMPLE_MODE_STRICT not set");
      return;
    }
    ...
    if (wt) {
      ...
      if (kept.stdout.trim() !== "KEPT") {
        throw new Error("uninstall deleted ~/.opensession despite committed scratch work");
      }
  ...
  test("uninstall: leaves nothing but repos (STRICT)", async () => {
    if (!STRICT) {
